...
void Teste_Diaria::casoSucesso(){
    try{
        diaria->setPreco(VALOR_VALIDO);
        if(diaria->getPreco() != (std::round(std::stof(VALOR_VALIDO)*100)/100.0)){
            estado = FALHA;
        }
...
void Teste_Diaria::casoFalhaCaratereInvalido(){
    try{
        diaria->setPreco(VALOR_CARACTERE_INVALIDO);
        estado = FALHA;
    } catch(std::invalid_argument &e){
        return;
    }
...
void Teste_Diaria::casoFalhaIntervaloInvalido(){
    try{
        diaria->setPreco(VALOR_INTERVALO_INVALIDO);
        estado = FALHA;
    } catch(std::invalid_argument &e){
        return;
    }
